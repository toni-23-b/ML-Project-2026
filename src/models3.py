...
print(f"\nOptimal threshold from ROC: {optimal_threshold:.4f}")

# pick threshold where recall (tpr) >= 0.7
for i in range(len(thresholds)):
    if tpr[i] >= 0.7:
        recall_threshold = thresholds[i]
        print(f"Threshold for Recall >= 0.7: {recall_threshold:.4f}")
        break

PREDICTION_TRIGGER = recall_threshold
